...
async function uploadFile(buffer, filename, mimeType = 'application/pdf') {
  const boundary = '----WebKitFormBoundaryCheckUnlock';
  let body = '';
  body += `--${boundary}\r\n`;
...
async function testUnlock() {
  ...
  pDoc.addPage().drawText('Protect & Unlock Test Page', { x: 50, y: 700 });
  const buf = Buffer.from(await pDoc.save());

  const u = await uploadFile(buf, 'TestDoc.pdf');
  ...
  const pRes = await makeRequest({
    ...
    headers: { 'Content-Type': 'application/json', 'x-session-id': 'unlock_check' }
  }, JSON.stringify({ fileId: u.fileId, password: 'mysecretpassword' }));

  console.log('Protect Response:', pRes.statusCode, pRes.body);

  const unRes = await makeRequest({
    hostname: 'localhost', port: 8092, path: '/api/pdf/unlock', method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-session-id': 'unlock_check' }
  }, JSON.stringify({ fileId: pRes.json.fileId, password: 'mysecretpassword' }));

  console.log('Unlock Response:', unRes.statusCode, unRes.body);
}
